...
/**
Model for meta-analyis where we add epoch as a random effect with the following characteristics:
...
data {
  ...
  int<lower=1> K_epoch;                        // Numbers of epochs
  ...
  int<lower=1,upper=K_epoch> epoch[Ntot];      // epoch index
...
parameters {
  ...
  real<lower=0> sigma_slope_epoch;     // variance of slope epoch random effects
  real<lower=0> sigma_intercept_epoch; // variance of intercept epoch random effects
  ...
  real theta_rand_slope_epoch[K_epoch];     // epoch random effects vector for slope
  real theta_rand_intercept_epoch[K_epoch]; // epoch random effects vector for intercept
...
transformed parameters {
  ...
  vector[Ntot] alpha_cov;
  {
    ...
    for(i in 1:Ntot){
      pred_log10_vl[i] =
      alpha_0 + theta_rand_id[id[i]][1] + alpha_cov[i] + gamma_rnasep*RNaseP[i] + theta_rand_intercept_epoch[epoch[i]] +
      beta_0*exp(trt_slope[i]+theta_rand_id[id[i]][2]+beta_cov[i]+theta_rand_slope_epoch[epoch[i]])*obs_day[i];
    }
  }
}
...
model {
  ...
  sigma_slope_epoch ~ exponential(1);
  sigma_intercept_epoch ~ exponential(1);
  ...
  theta_rand_slope_epoch ~ normal(0,sigma_slope_epoch);
  theta_rand_intercept_epoch ~ normal(0,sigma_intercept_epoch);
...
generated quantities {
  ...
  for(i in 1:n_id){
    int j = ind_start[i];
    slope[i] = beta_0*exp(trt_slope[j]+theta_rand_id[id[j]][2]+beta_cov[j]+theta_rand_slope_epoch[epoch[i]]);
  }
}
