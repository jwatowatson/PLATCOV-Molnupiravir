...
transformed parameters {
  ...
  
  {
    ...
    for(i in 1:Ntot){
      real intercept = alpha_0 + theta_rand_id[id[i]][1] + alpha_cov[i] + theta_rand_intercept_epoch[epoch[i]];
      real a = beta_0[1] * exp(trt_slope[i]); // no random effect for the first term as does not converge
      real b = beta_0[2] * exp(theta_rand_id[id[i]][2] + trt_slope[i] + beta_cov[i] + theta_rand_slope_epoch[epoch[i]]);
...
generated quantities {
  ...
  for(i in 1:n_id){
    int j = ind_start[i];
    slope[i] = -beta_0[2] * exp(theta_rand_id[id[j]][2] + trt_slope[j] + beta_cov[j] + theta_rand_slope_epoch[epoch[i]]);
  }
}
